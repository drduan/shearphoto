<?php
/*************ShearPhoto1.1 免费，开源，兼容目前所有浏览器，纯原生JS和PHP编写,完美兼容linux和WINDOW服务器*********
...
function errobug() 
{ $e = error_get_last(); 
$e["type"]>0  and $e['type'] != 8 and die('{"erro":"服务器错误或超时"}');
}
register_shutdown_function('errobug');//注册FUNCTION,接收系统致命错误
error_reporting(0);//关闭错误提示
ini_set('max_execution_time', 30);//设置超时5秒
$filename = $ShearPhoto["config"]["temp"].DIRECTORY_SEPARATOR."temp_".time()."_".mt_rand(1000000,99999999).".jpg";
$phpinp=file_get_contents('php://input');
$furl=ShearURL."file".DIRECTORY_SEPARATOR;
$phpinpsize=strlen($phpinp);
if($phpinpsize==8351){//摄象头没启动
$erroimg_one=file_get_contents($furl.'camerro/erro1.jpg');
if($phpinp==$erroimg_one){
die('{"erro":"请允许摄象头启动"}');	
}
}

$result = file_put_contents($filename, $phpinp);
if (!$result) {
die('{"erro":"拍照错误"}');
}

echo('{"success":"'.str_replace(array("\\\\","\/",ShearURL,"\\"),array("\\","/","","/"),$filename).'"}');
?>
